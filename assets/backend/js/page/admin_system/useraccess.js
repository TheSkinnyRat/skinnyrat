// <!-- WARNING: DATA TABLES SCRIPT -->
  ...
  $(document).ready(function(){
  	...
  	$('#px-system-useraccess-message-form').validate({
  		...
  		submitHandler: function(form) {
  			var target = $(form).attr('action');
  			$('#px-system-useraccess-message-form .msg-status').text('Sedang menghapus data');
  			$.ajax({
  				url : target,
  				type : 'POST',
  				dataType : 'json',
  				...
  				success : function(response){
  					if(response.status == 'ok'){
  						$('#px-system-useraccess-message-form .msg-status').text('Data berhasil dihapus, anda akan dialihkan');
  						window.location.href = response.redirect;
  					}
  					else
  						$('#px-system-useraccess-message-form .msg-status').text('Data gagal dihapus');
  				},
  				error : function(jqXHR, textStatus, errorThrown) {
  					alert(textStatus, errorThrown);
  				}
  	...
  	$('.btn-add').click(function(){
  		...
  		$('#px-system-useraccess-form [type="checkbox"]').prop('checked',false);
  		$('#px-system-useraccess-group-label').remove();
  		$('#px-system-useraccess-modal').modal('show');
      $('#px-system-useraccess-form-id-usergroup').removeClass('hidden');
  	});
